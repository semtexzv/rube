...
pub fn register_types(registry: &mut reflect::Registry) {
    registry.register(&UnitDeployment::default());
...
pub struct UnitDeployment {
    pub unit: Option<Box<Ref>>,
    pub targets: Vec<String>,
    pub strategy: Option<UnitDeploymentStrategy>,
    pub _unknown: (),
}
impl UnitDeployment {
    #[inline(always)]
    pub fn r#with_unit(mut self, it: Ref) -> Self {
        self.r#set_unit(it);
        self
    }
    #[inline(always)]
    pub fn r#set_unit(&mut self, it: Ref) -> &mut Self {
        self.unit = Box::new(it).into();
        self
    }
    #[inline(always)]
    pub fn r#with_targets(mut self, it: String) -> Self {
        self.r#add_targets(it);
        self
    }
    #[inline(always)]
    pub fn r#add_targets(&mut self, it: String) -> &mut Self {
        self.targets.push(it);
        self
    }
    #[inline(always)]
    pub fn r#with_strategy(mut self, it: UnitDeploymentStrategy) -> Self {
        self.r#set_strategy(it);
        self
    }
    #[inline(always)]
    pub fn r#set_strategy(&mut self, it: UnitDeploymentStrategy) -> &mut Self {
        self.strategy = it.into();
        self
    }
}
impl textformat::Decodable for UnitDeployment {
    fn merge_field(
        &mut self,
        ...
        match name {
            textformat::ast::FieldName::Normal("unit") => {
                textformat::Field::merge(&mut self.unit, ctx, value)?;
            }
            textformat::ast::FieldName::Normal("targets") => {
                textformat::Field::merge(&mut self.targets, ctx, value)?;
            }
            textformat::ast::FieldName::Normal("strategy") => {
                textformat::Field::merge(&mut self.strategy, ctx, value)?;
            }
...
impl textformat::Encodable for UnitDeployment {
    fn encode(
        &self,
        ctx: &textformat::Context,
        pad: usize,
        ...
        if self.unit != <Option<Box<Ref>> as Default>::default() {
            out.indent(pad);
            out.push_str("unit ");
            textformat::Field::format(&self.unit, ctx, pad, out)?;
            out.push('\n');
        }
        if self.targets != <Vec<String> as Default>::default() {
            out.indent(pad);
            out.push_str("targets: ");
            textformat::Field::format(&self.targets, ctx, pad, out)?;
            out.push('\n');
        }
        if self.strategy != <Option<UnitDeploymentStrategy> as Default>::default() {
            out.indent(pad);
            out.push_str("strategy: ");
            textformat::Field::format(&self.strategy, ctx, pad, out)?;
            out.push('\n');
        }
        Ok(())
    }
}
impl binformat::Decodable for UnitDeployment {
    fn merge_field<'i, 'b>(
        &'i mut self,
        tag: u32,
        mut buf: &'b [u8],
        ...
        match tag {
            10u32 => {
                buf = Decode::<Nest>::decode(&mut self.unit, buf)?;
            }
            18u32 => {
                buf = Decode::<Repeat::<Bytes>>::decode(&mut self.targets, buf)?;
            }
            24u32 => {
                buf = Decode::<Enum>::decode(&mut self.strategy, buf)?;
            }
            26u32 => {
                buf = Decode::<Enum>::decode(&mut self.strategy, buf)?;
            }
...
impl binformat::Encodable for UnitDeployment {
    fn qualified_name(&self) -> &'static str {
        "api.systemd.UnitDeployment"
    }
    fn encode(&self, buf: &mut binformat::Buffer) -> binformat::Result<()> {
        use binformat::format::*;
        Decode::<Nest>::encode(&self.unit, 10u32, buf)?;
        Decode::<Repeat::<Bytes>>::encode(&self.targets, 18u32, buf)?;
        Decode::<Enum>::encode(&self.strategy, 24u32, buf)?;
...
#[derive(Debug, Clone, PartialEq)]
pub enum UnitDeploymentStrategy {
    FILE,
    TRANSIENT,
    Unknown(u32),
}
impl Default for UnitDeploymentStrategy {
    fn default() -> UnitDeploymentStrategy {
        Self::from(0)
    }
}
impl binformat::format::ProtoEnum for UnitDeploymentStrategy {}
impl From<u32> for UnitDeploymentStrategy {
    fn from(v: u32) -> UnitDeploymentStrategy {
        match v {
            0u32 => UnitDeploymentStrategy::FILE,
            1u32 => UnitDeploymentStrategy::TRANSIENT,
            other => UnitDeploymentStrategy::Unknown(other),
        }
    }
}
impl Into<u32> for UnitDeploymentStrategy {
    fn into(self) -> u32 {
        match self {
            UnitDeploymentStrategy::FILE => 0u32,
            UnitDeploymentStrategy::TRANSIENT => 1u32,
            UnitDeploymentStrategy::Unknown(other) => other,
        }
    }
}
impl textformat::Field for UnitDeploymentStrategy {
    fn format(
        &self,
        ctx: &textformat::Context,
        pad: usize,
        ...
        let str = match self {
            UnitDeploymentStrategy::FILE => "FILE",
            UnitDeploymentStrategy::TRANSIENT => "TRANSIENT",
            UnitDeploymentStrategy::Unknown(n) => {
                write!(out, "{n}")?;
                return Ok(());
            }
        };
    ...
    fn merge_scalar(
        ...
        match v {
            textformat::ast::Literal::Identifier("FILE") => {
                *self = UnitDeploymentStrategy::FILE;
            }
            textformat::ast::Literal::Identifier("TRANSIENT") => {
                *self = UnitDeploymentStrategy::TRANSIENT;
            }
